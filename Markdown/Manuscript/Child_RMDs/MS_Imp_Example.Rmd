...
Using the framework website as a template, we used the search and documentation tools from our foundation framework library to create and help maintain a project documentation website (https://readyforwhatsnext.org/). The project website includes details on available model modules, datasets and analysis programs and tutorials on how these artefacts can be used. 

## Transferability assessment
We subjectively assessed these utility mapping model artefacts against <!-- Edit_start -->TRU criteria <!-- Edit_end --> (Table \@ref(tab:checktb)). For each criterion, we provided a global assessment of whether it was met using the responses “yes”, “no” or “partial”. We believe that our utility mapping study module libraries, code and data repositories and documentation have satisfactorily met four of the six criteria (T1, R1, R2 and U1) and have partially met two criteria (T2 and U2). The main shortcomings that we identified when applying the assessment criteria was that individual modules and functions have yet to: (i) be supported with human authored documentation that includes examples of how they can be used; and (ii) be quality assured through unit testing. 
