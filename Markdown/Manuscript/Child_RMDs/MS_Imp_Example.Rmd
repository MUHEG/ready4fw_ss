...
MUR 3) *Multi-attribute instrument scoring tools* that: (i) attach structured metadata about an instrument (e.g. name, version, country, items, domains, parameter values for scoring algorithms) to a human records dataset; (ii) apply pre-written (for AQol-6D and EQ-5D instruments) or user-supplied instrument scoring functions to a human records dataset; and (iii) generate descriptive plots of a dataset's instrument scores for individual items, domains and totals (weighted and unweighted).
...
We used using the search and documentation tools from our foundation framework library to
create and help maintain a documentation website (https://readyforwhatsnext.org/) that includes details on available model modules, datasets and analysis programs and tutorials on using model modules. 

## Transferability assessment
We subjectively assessed these utility mapping model artefacts against <!-- Edit_start -->our TRU criteria <!-- Edit_end --> (Table \@ref(tab:checktb)). For each criterion, we provided a global assessment of whether it was met using the responses “yes”, “no” or “partial”.  We believe our utility mapping study modules have satisfactorily met four of the six criteria (T1, R1, R2 and U1) and have partially met two criteria (T2 and U2). The main shortcomings that we identified when applying the assessment criteria was that we have yet to: (i) ensure that module library documentation includes examples of how to apply all exported methods and functions; and (ii) adequately implement unit testing. 
