...
We have identified 20 standards that we believe are important for quality implementations of OSHEMs, each described under one of following six principles for making models TIMELY:
 ...
 - **Epitomized**: a model and its components can be used in multiple decision contexts;
 - **Licensed**: a model, its components and derivatives are persistently re-usable by other modellers; and
 - **Yielding**: a model can be simply, flexibly and reliably used to inform decision-making.
...
Consistent use of meaningful naming conventions when authoring code is recommended [@Wilson_2017; @Alarid2019]. Code can be made easier to follow by using the practices of abstraction [@8717448], where only simple, high level commands are routinely exposed to reviewers, and polymorphsim [@7181447], where the same command (e.g. "simulate") can be reused to implement different algorithms of the same type. Programs to implement model analyses can be made comprehensible to even non-technical users though the use of literate programming techniques that use tools like RMarkdown [@xie2018r] to render documents that integrate computer code with plain English descriptions. 
...
- self-documenting model modules [@ready4class2022];

- self-documenting functions (including methods), written in a consistent house style [@ready4fun2022];
...
The six R packages, their primary focus, the TIMELY standards they support and the third party packages they depend on are summarised in Table \@ref(tab:cpkgs). When used in conjunction with framework repositories, the six packages currently provide strong support for implementing 17 of the TIMELY standards, weak support for implementing the standards relating to disseminating statistical models (Y1) and user-interface development (Y2) and no support for implementing standard T5 (reporting code coverage). Our future development priorities for these packages include adding functionality so that all 20 TIMELY standards are well supported and to then submit production versions of each package for review by and archiving on the Comprehensive R Archive Network (CRAN) [@CRAN2022].
...
```{r tab.cap='TIMELY Checklist applied to utility mapping study', tab.id = 'checklisttb', echo = F, results = 'asis'}
    #  a <- c("TEST1","TEST2")
...
checklist_df <- data.frame(Ref = c(paste0("T",1:6),
                                        paste0("I",1:4),
                                        paste0("M",1:2),
                                        paste0("E",1:3),
                                        paste0("L",1:3),
                                        paste0("Y",1:2)),
                           Standard = c("Publicly archived",
                                        "Documented",
                                        "Consistent syntax",
                                        ...
                                        "Test data",
                                        "Copyleft code",
                                        "Liberal, safe data terms",
                                        "Citation tools",
                                        "Prediction tools",
                                        "User interface"),
                           Assessment = rep("F",20),
                           Description = c("5 libraries (youthvars, scorz, specific, TTU and youthu) [@hamilton_matthew_2022_6084467; @hamilton_matthew_2022_6084824; @hamilton_matthew_2022_6116701; @gao_caroline_2022_6130155; @matthew_p_hamilton_2021_5646669], <br> 3 programs [@hamilton_matthew_2022_6129906; @hamilton_matthew_2022_6416330; @hamilton_matthew_p_2022_6321821], 2 sub-routines [@hamilton_matthew_2022_6116385; @matthew_p_hamilton_2022_5976988] and 2 datasets [@DVN/DKDIB0_2021; @DVN/HJXYKQ_2021] are permanently archived with unique identifiers.",
                                           ...
                                           "All libraries, programs and sub-routines use the same house style. All libraries except [@matthew_p_hamilton_2021_5646669] use framework syntax, as does one program [@hamilton_matthew_2022_6129906].",
                                           ...
                                           "All code is version controlled using Git and GitHub. All source code is available in one of two GitHub organisations [@ready4gh2022; @orygengh2022].",
                                           ...
                                           "All six libraries use continuous integration (CI). CI results for each library can be viewed at URL that concatenates 'https://github.com/ready4-dev/' the package name and '/actions' (e.g. https://github.com/ready4-dev/youthvars/actions)",
                                           "Retired code is deprecated using lifecycle package tools (e.g. review everything after “## DEPRECATED FNS” in https://github.com/ready4-dev/youthvars/blob/main/data-raw/fns/add.R . Package vignettes and datasets are also deprecated e.g. https://ready4-dev.github.io/youthvars/articles/Replication_DS.html )",
                                           "All development code is stored on repos in two GitHub organisations [@ready4gh2022; @orygengh2022] and all archived releases are available in aZenodo community [@ready4zen2022]. All non-confidential data is stored in repositories within a Harvard Dataverse collection [@ready4dv2022]. ",
                                           "Four [@hamilton_matthew_2022_6084467; @hamilton_matthew_2022_6084824; @hamilton_matthew_2022_6116701; @gao_caroline_2022_6130155] out of five libraries include encapsulated  modules. Examples are the items beginning with Scorz, Specific and Youthvars that are listed in this table: https://ready4-dev.github.io/ready4/articles/V_01.html#current-ready4-framework-modules as well as the S4 classess from the TTU package listed here: https://ready4-dev.github.io/TTU/reference/index.html ",
                                           ...
                                           "All modules (see item M2) inherit from the Ready4Module class.",
                                           "Two synthetic datasets and their data dictionaries are publicly available in a data repository [@DVN/HJXYKQ_2021]. One (ymh_clinical_tb.RDS) closely resembles the study dataset and was released so that the main study algorithm [@hamilton_matthew_2022_6129906] can be rerun by those without access to the confidential data. The other (eq5d_ds_dict.RDS) is deliberately different to the source dataset in both variable naming convention and the concepts used for predictors and outcome measures and was created to demonstrate generalised applications of study algorithms.",
                                           ...
                                           "All libraries have CITATION file in inst directory. All code repositories have CITATION.cff file. All datasets have citation generating metadata.",
                                           "Model catalogues (PDF files beginning with 'AAA_TTU_MDL_CTG') describe the predictive performance of all models under a variety of usage regimes and are available in the study results dataset [@DVN/DKDIB0_2021]. The youthu library [@matthew_p_hamilton_2021_5646669] includes tools for searching for and applyihg models compatible with different types of input data. An example program to demonstrate this functionality is available in RMarkdown [@hamilton_matthew_2022_6416330] and rendered PDF formats (the 'Application.pdf' file in the study results dataset [@DVN/DKDIB0_2021]).  ",
...
checklist_df <- data.frame(checklist_df[,1],
                 rep("&nbsp; ",nrow(checklist_df)),
                 checklist_df[,2],
                 rep("&nbsp; &nbsp; &nbsp; &nbsp; ",nrow(checklist_df)),
                 checklist_df[,3],
                 rep("&nbsp; ",nrow(checklist_df)),
                 checklist_df[,4])
colnames(checklist_df) <- c(" "," ","Standard",#
                             "   ","Met?","   ",
                            tb2_nms_chr[4])
