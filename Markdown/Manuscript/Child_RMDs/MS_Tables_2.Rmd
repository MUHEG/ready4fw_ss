...
---
```{r checktb, tab.cap='TIMELY Checklist applied to utility mapping study', tab.id = 'checktb', echo = F, results = 'asis'}
    #  a <- c("TEST1","TEST2")
...
checklist_df <- data.frame(Ref = c(paste0("T",1:6),
                           ...
                           Description = c("5 libraries (youthvars, scorz, specific, TTU and youthu) [@hamilton_matthew_2022_6084467; @hamilton_matthew_2022_6084824; @hamilton_matthew_2022_6116701; @gao_caroline_2022_6130155; @matthew_p_hamilton_2021_5646669], <br> 3 programs [@hamilton_matthew_2022_6129906; @hamilton_matthew_2022_6416330; @hamilton_matthew_p_2022_6321821], 2 sub-routines [@hamilton_matthew_2022_6116385; @matthew_p_hamilton_2022_5976988] and 2 datasets [@DVN/DKDIB0_2021; @DVN/HJXYKQ_2021] are permanently archived with unique identifiers.",
                                           ...
                                           "All modules (see item M2) inherit from the Ready4Module class and can be inherited from.",
                                           "Two synthetic datasets and their data dictionaries are publicly available in a data repository [@DVN/HJXYKQ_2021]. One (ymh_clinical_tb.RDS) closely resembles the study dataset and was released so that the main study algorithm [@hamilton_matthew_2022_6129906] can be rerun by those without access to the confidential study dataset. The other (eq5d_ds_dict.RDS) is deliberately different to the source dataset in both variable naming convention and the concepts used for predictors and outcome measures and was created to demonstrate generalised applications of study algorithms.",
                                           ...
                                           "All libraries have CITATION file in inst directory. All code repositories have a CITATION.cff file. All datasets have citation generating metadata.",
                                           "Model catalogues (PDF files beginning with 'AAA_TTU_MDL_CTG') are available in the study results dataset [@DVN/DKDIB0_2021] and describe the predictive performance of all models under a variety of usage regimes (including when the source dataset in the R model object is replaced with fake data). The youthu library [@matthew_p_hamilton_2021_5646669] includes tools for searching for and applyihg models compatible with different types of input data. An example program to demonstrate this functionality is available in both RMarkdown [@hamilton_matthew_2022_6416330] and rendered PDF formats (the 'Application.pdf' file in the study results dataset [@DVN/DKDIB0_2021]).  ",
