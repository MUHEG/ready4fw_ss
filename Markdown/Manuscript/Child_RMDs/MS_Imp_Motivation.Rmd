...
## Economic topics
Initially, we plan to construct readyforwhatsnext from projects in four of the twelve domains of health economics identified by Wagstaff and Culyer [@wagstaff2012four]: 
...
A high level schematic of how we aim to implement readyforwhatsnext as a modular and open source CHEM to address these topics is outlined in Figure \@ref(fig:fig1). 

```{r, child = child_figs_ls$fig1, echo=FALSE}
```

## User requirements
Although we are principally interested in using readyforwhatsnext to answer policy questions relating to the mental health of young people in Australia, we want to facilitate model transfer to other jurisdictions. We believe that a good starting point for facilitating model transferable is to develop a CHEM that is relatively easy to understand, use and adapt. For that reason, the initial user-requirements we specified for readyforwhatsnext are to meet six explicit criteria for transparency, reusability and updatability. We have describe these criteria elsewhere [@ethicsCHEMS2024], but briefly they are: 
...
T4) Licenses restrict only unethical reuse; <!-- rephrase Liceses permit derivative works-->

T5) Maintenance infrastructure is in place; and 

T6) New releases are systematically retested, with deprecation of outdated artefacts.
