...
UR 1) A *template CHEM module* which can be used to create CHEM modules that can: (i) safely share inputs and outputs with each other without impairing any module's integrity; (ii) be run as independent models; and (iii) be selectively replaced, updated or extended.

UR2) *Tools* for creating new CHEM modules from the template module. 

UR 3) A simple *syntax* to support consistency in how CHEM modules are used in health economic analysis programs.
...
UR 5) Integration with online open science data archiving services to streamline the *ingesting, labeling and sharing of CHEM data*.

UR 6) Tools to help *author replicable health economic analysis programs and reporting subroutines*.
